...
def anomaly_detection(data_type, delta, ratio, sample_method, update_method, model):

    print('-------------------' + str(delta) + '-----------------------')
    print('-------------------' + str(ratio) + '-----------------------')

    for channel in data_type:

        with open('./results/' + str(channel) + '_update' + str(update_method) + '_' + str(sample_method) + '_ratio=' + str(
                ratio) + '_delta=' + str(delta) + '_cnn.txt', 'w') as f:
            ...
            opt.dataset = channel
            opt.update_method = update_method
            ...
            path_train = os.path.join(os.getcwd(), "datasets", "train", channel)
            ...
            for file in files:
                ...
                if opt.sample_method == 'seq':
                    samples_train_data, samples_val_data = read_train_data_seq_based(opt.window_size, file=data_name,
                                                                       step=opt.step, delta=delta, ratio=ratio)
                ...
                print(str(channel) + '\t' + str(file) + '\tf1=' + str(f1) + '\tpre=' + str(pre) +
                ...
                print('total results:' + '\tt_f1=' + str(total_f1) + '\tt_pre=' + str(total_pre) +
                      ...
                      + '\tt_epoch_time=' + str(total_epoch_time) + '\tt_test_time=' + str(total_test_time/file_number/opt.test_batchsize))

                f.write(str(channel) + '\t' + str(file) + '\t' + str(f1) + '\t' + str(pre) + '\t' +
                        str(rec) + '\t' + str(tp) + '\t' + str(tn) + '\t' + str(fp) + '\t' + str(fn) +
                        '\t' + str(train_time) + '\t' + str(epoch_time) + '\t' + str(test_time) +
                        '\t' + str(latency) + '\n')
            f.write('\n')
            f.write('total results' + '\t' + str(channel) + '\t' + str(total_f1) + '\t' + str(total_pre) + '\t' +
                    str(total_rec) + '\t' + str(total_tp) + '\t' + str(total_tn) + '\t' + str(total_fp) + '\t' + str(total_fn) +
                    '\t' + str(total_train_time) + '\t' + str(total_epoch_time) + '\t' + str(total_test_time/file_number/opt.test_batchsize) +
                    '\t' + str(total_latency) + '\n')
...
if __name__ == '__main__':
    ...
    commands = sys.argv[1:]
    #data_type(SMD or SMAP or MSL or SWAT), delta, ratio, sample_method(seq or win or sw), update_method(1 or 2), ‘ratio’ or ‘delta’
    anomaly_detection(commands[0], int(commands[1]), int(commands[2]), commands[3], int(commands[4]), commands[5])
